...
export default function ErrorState({ 
  ...
  return (
    <div className={`bg-slate-900/60 rounded-2xl border border-rose-900/50 p-8 flex flex-col items-center justify-center text-center shadow-lg ${minHeight}`}>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        {supportLink && (
          <Button variant="secondary" onClick={() => window.open(supportLink, '_blank')} iconLeft={<ExternalLink className="w-4 h-4" />}>
            Contact Support
          </Button>
        )}
        {onRetry && (
          <Button variant="primary" onClick={onRetry} iconLeft={<RefreshCw className="w-4 h-4" />}>
            Retry Request
          </Button>
        )}
      </div>
