...
export default function Drawer({ isOpen, onClose, title, children, width = 'w-full md:w-[600px] lg:w-[800px]' }) {
  ...
  return createPortal(
    <>
      ...
      {/* Drawer Panel */}
      <div 
        className={`fixed top-0 right-0 bottom-0 z-50 bg-card border-l border-border shadow-2xl flex flex-col transition-transform duration-300 ease-in-out transform ${width} ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {/* Header */}
        {title && (
