  ...
  return (
    ...
    >
      ...
      <input
        ...
        className={`w-full ${sizeConfig.container} ${sizeConfig.padding} rounded-xl bg-muted border border-border text-slate-900 dark:text-slate-200 placeholder-slate-500 dark:placeholder-slate-400 focus:outline-none focus:border-indigo-500/60 focus:ring-1 focus:ring-indigo-500/30 transition-all ${
      ...
      {value && onClear ? (
        ...
        <kbd className={`absolute ${sizeConfig.iconRightPos} px-1.5 py-0.5 text-xs font-mono text-muted-foreground bg-slate-200/80 dark:bg-slate-800/80 border border-slate-300/80 dark:border-slate-700/60 rounded pointer-events-none`}>
          {shortcut}
        </kbd>
      ) : null}
    </div>
