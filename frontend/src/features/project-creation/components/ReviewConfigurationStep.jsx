    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs max-h-[340px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-800">
        ...
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 dark:bg-slate-950/70 dark:border-slate-800 space-y-3">
          <div className="flex items-center gap-2 font-semibold text-slate-900 dark:text-slate-200 text-xs border-b border-border pb-2">
        ...
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 dark:bg-slate-950/70 dark:border-slate-800 space-y-3">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-muted-foreground">{rootDirectory || '/'}</span>
            </div>

            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">Node Version:</span>
              <span className="font-mono text-muted-foreground">Node {nodeVersion}</span>
            </div>
          ...
          <div className="space-y-1.5">
            {envVars.filter((e) => e.key.trim()).length === 0 ? (
              ...
              envVars
                .filter((e) => e.key.trim())
                .map((env) => (
                  <div key={env.id} className="flex items-center justify-between text-sm bg-card p-1.5 rounded border border-border">
