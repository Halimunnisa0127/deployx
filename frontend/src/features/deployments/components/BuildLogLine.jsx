...
function renderHighlightedText(text, logType) {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (val === '✓' || val === 'DONE' || val === 'SUCCESS') {
      ...
      style = 'text-rose-400 font-bold bg-rose-950/60 px-1 rounded';
    } else if (val.toUpperCase().includes('WARN')) {
      style = 'text-amber-400 font-semibold';
    } else if (val.startsWith('http')) {
      style = 'text-sky-400 underline hover:text-sky-300';
    } else if (val.match(/\.(js|html|css|json|zip|png|svg|ts|jsx|tsx)$/)) {
      style = 'text-purple-300 font-medium font-mono';
    } else if (val.match(/\d+(?:\.\d+)?\s*(?:kB|MB|B|GB|ms|s)/)) {
      style = 'text-amber-300 font-mono font-medium';
    }
  ...
  let baseColor = 'text-slate-300';
  if (logType === 'success') baseColor = 'text-emerald-300 font-medium';
  if (logType === 'warning') baseColor = 'text-amber-300 font-medium';
  if (logType === 'error') baseColor = 'text-rose-300 font-semibold';
  if (logType === 'debug') baseColor = 'text-slate-400';
