  ...
  return (
    <div className="space-y-4">
      {commits.map((commit, index) => (
        <div key={commit.id} className="relative flex items-start gap-4 p-4 rounded-xl hover:bg-slate-800/30 transition-colors group">
          {/* Vertical Timeline Line */}
          {index !== commits.length - 1 && (
            <div className="absolute left-8 top-12 bottom-0 w-px bg-slate-800 group-hover:bg-indigo-500/30 transition-colors" />
          )}
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <span className="text-xs font-semibold text-slate-400">{commit.author}</span>
