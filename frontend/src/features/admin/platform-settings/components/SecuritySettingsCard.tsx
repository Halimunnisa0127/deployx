...
export default function SecuritySettingsCard({ register, watch, setValue }) {
  ...
  return (
    <div id="security" className="bg-slate-900/60 rounded-2xl border border-slate-800/80 p-6 shadow-lg mb-10">
      <div className="mb-6 border-b border-slate-800/80 pb-4">
        <h2 className="text-lg font-bold text-white">Security Settings</h2>
        <p className="text-sm text-slate-400">Manage authentication and access policies.</p>
      </div>
      ...
      <div className="space-y-6 max-w-3xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">API Rate Limit (req/min)</label>
            <Input 
        ...
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1.5">Password Policy</label>
          <select 
          ...
          >
            ...
            <option value="medium">Medium (8+ chars, 1 number, 1 special)</option>
            <option value="strong">Strong (12+ chars, mixed case, numbers, special)</option>
          </select>
        </div>

        <div className="flex items-center justify-between p-4 bg-slate-900/40 border border-slate-800/80 rounded-xl">
          <div>
            <div className="font-semibold text-slate-200">Enforce Two-Factor Authentication (2FA)</div>
            <div className="text-sm text-slate-400">Require all administrative users to enable 2FA.</div>
          </div>
          <button
            type="button"
            onClick={() => setValue('security.require2fa', !require2fa, { shouldDirty: true })}
