...
export default function MaintenanceSettingsCard({ register, watch, setValue }) {
  ...
  return (
    <div id="maintenance" className="bg-slate-900/60 rounded-2xl border border-slate-800/80 p-6 shadow-lg">
      <div className="mb-6 border-b border-slate-800/80 pb-4">
        <h2 className="text-lg font-bold text-white">Maintenance Mode</h2>
        <p className="text-sm text-slate-400">Lock the platform for users during upgrades.</p>
      </div>

      <div className="space-y-6 max-w-2xl">
        <div className="flex items-center justify-between p-4 bg-slate-900/80 border border-slate-700/80 rounded-xl">
          <div>
            <div className="font-semibold text-slate-200">Enable Maintenance Mode</div>
            <div className="text-sm text-slate-400">Non-admin users will be redirected to the maintenance page.</div>
          </div>
          <button
            type="button"
        ...
        <div className={`space-y-6 transition-opacity duration-300 ${isEnabled ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Maintenance Message</label>
            <textarea 
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Allowed IP Addresses</label>
            <Input 
              {...register('maintenance.allowedIps')}
              placeholder="192.168.1.1, 10.0.0.1"
              className="w-full bg-slate-900 border-slate-700"
            />
            <p className="mt-1.5 text-xs text-slate-500">Comma separated list of IPs allowed to bypass maintenance mode.</p>
          </div>
        </div>
      </div>
