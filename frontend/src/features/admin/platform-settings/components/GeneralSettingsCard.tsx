...
export default function GeneralSettingsCard({ register, errors }) {
  return (
    <div id="general" className="bg-slate-900/60 rounded-2xl border border-slate-800/80 p-6 shadow-lg">
      <div className="mb-6 border-b border-slate-800/80 pb-4">
        <h2 className="text-lg font-bold text-white">General Settings</h2>
        <p className="text-sm text-slate-400">Configure primary platform identification details.</p>
      </div>

      <div className="space-y-6 max-w-2xl">
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1.5">Platform Name <span className="text-rose-500">*</span></label>
          <Input 
          ...
          {errors?.general?.platformName && (
            <p className="mt-1.5 text-sm text-rose-500">{errors.general.platformName.message}</p>
          )}
        </div>

        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1.5">Default Region</label>
          <select 
