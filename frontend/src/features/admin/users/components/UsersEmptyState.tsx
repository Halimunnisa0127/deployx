...
export function NoUsersEmptyState({ onAddUser }) {
  return (
    <div className="bg-slate-900/60 rounded-2xl border border-slate-800/80 p-10 flex flex-col items-center justify-center text-center">
      ...
      <p className="text-slate-400 max-w-sm mb-6">You don't have any users in your platform yet. Start by inviting a new user.</p>
      <Button variant="primary" onClick={onAddUser}>Invite User</Button>
    </div>
  );
}
...
export function NoSearchResultsEmptyState({ onClear }) {
  return (
    <div className="bg-slate-900/60 rounded-2xl border border-slate-800/80 p-10 flex flex-col items-center justify-center text-center">
      ...
      <p className="text-slate-400 max-w-sm mb-6">No users matched your search or filter criteria. Try adjusting them.</p>
...
export function NoActiveUsersEmptyState({ onClear }) {
  return (
    <div className="bg-slate-900/60 rounded-2xl border border-slate-800/80 p-10 flex flex-col items-center justify-center text-center">
      ...
      <p className="text-slate-400 max-w-sm mb-6">There are currently no active users on the platform.</p>
      <Button variant="secondary" onClick={onClear}>Clear Filters</Button>
    </div>
  );
}
...
export function NoSuspendedUsersEmptyState({ onClear }) {
  return (
    <div className="bg-slate-900/60 rounded-2xl border border-slate-800/80 p-10 flex flex-col items-center justify-center text-center">
      ...
      <p className="text-slate-400 max-w-sm mb-6">There are currently no suspended users on the platform.</p>
      <Button variant="secondary" onClick={onClear}>Clear Filters</Button>
    </div>
  );
}
