...
export default function ProjectsPage() {
  ...
  const counts = useMemo(() => {
    const res = { all: projects.length, active: 0, archived: 0, failed: 0, 'React': 0, 'Next.js': 0, 'Node.js': 0 };
    projects.forEach(p => {
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter(p => {
      ...
      if (query) {
        return p.name.toLowerCase().includes(query) || p.owner.toLowerCase().includes(query);
      }
      return true;
    });
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-10 text-left animate-in fade-in duration-300">
      ...
      {loading ? (
        ...
        activeFilter === 'active' ? <NoActiveProjectsEmptyState onClear={() => setActiveFilter('all')} /> :
        activeFilter === 'archived' ? <NoArchivedProjectsEmptyState onClear={() => setActiveFilter('all')} /> :
