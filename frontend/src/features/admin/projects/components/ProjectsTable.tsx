...
export default function ProjectsTable({ projects = [], onRowClick, actionHandlers }) {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;
  
  const totalPages = Math.ceil(projects.length / itemsPerPage);
  const paginatedData = projects.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
