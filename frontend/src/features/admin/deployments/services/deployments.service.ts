...
export const getDeploymentTimeline = async (id: string) => {
  await wait(300);
  return [
...
export const getDeploymentArtifacts = async (id: string) => {
  await wait(300);
  return {
    buildSize: '14.2 MB',
...
export const redeployDeployment = async (id: string) => {
  await wait(800);
  return { success: true, message: 'Redeployment triggered' };
};

export const cancelDeployment = async (id: string) => {
  await wait(500);
...
export const exportDeployments = async (format: string = 'csv') => {
  await wait(1000);
  return { success: true, message: `Exported deployments.${format} successfully` };
};
