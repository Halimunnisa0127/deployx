...
export default function AdminSidebar({ onToggleMobileExternal }: { onToggleMobileExternal?: () => void }) {
  ...
  const filteredNavItems = useMemo(() => {
    ...
    return ADMIN_NAV_ITEMS.filter((item) => item.label.toLowerCase().includes(query));
  }, [navSearchQuery]);
  ...
  return (
    <>
      <aside className={`hidden md:block sticky top-0 h-screen transition-all duration-300 ease-in-out ${sidebarWidth} z-30 flex-shrink-0`}>
        {sidebarContent}
      </aside>
