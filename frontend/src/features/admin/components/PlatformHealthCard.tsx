  ...
  return (
    <Card className="h-full flex flex-col p-5 sm:p-6 bg-slate-900/60 border-slate-800/80 shadow-lg">
      <div className="flex items-center justify-between mb-6">
        ...
        <Button variant="ghost" size="sm" iconLeft={<RefreshCw className="w-4 h-4 text-slate-400" />} className="text-slate-400 hover:text-white hover:bg-slate-800">
          Refresh
        </Button>
      </div>
      ...
      <div className="space-y-4 flex-1">
        {health.map((item) => {
          ...
          return (
            <div key={item.id} className="p-4 rounded-xl bg-slate-800/40 border border-slate-700/50 flex items-center justify-between group hover:bg-slate-800/60 transition-colors">
