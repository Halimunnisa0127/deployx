...
export const getDashboardAnalytics = async (dateRange: string) => {
  await wait(600);
  return {
...
export const getDeploymentTrend = async (dateRange: string) => {
  await wait(400);
  const data = [];
  const now = new Date();
  for (let i = 14; i >= 0; i--) {
...
export const getUserGrowth = async (dateRange: string) => {
  await wait(400);
  const data = [];
  const now = new Date();
  let baseUsers = 12000;
...
export const getProjectGrowth = async (dateRange: string) => {
  await wait(400);
  return [
...
export const exportReport = async (format: string) => {
  await wait(1200);
