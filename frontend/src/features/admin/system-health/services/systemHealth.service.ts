...
export const getPerformanceMetrics = async () => {
  ...
  const generateTrend = (base: number, variance: number, points = 20) => {
  ...
  return {
    ...
    connections: { current: 1250, trend: 120, data: generateTrend(1200, 200).map(d => ({ ...d, value: Math.floor(d.value * 25) })) },
    requests: { current: 4500, trend: 350, data: generateTrend(4000, 1000).map(d => ({ ...d, value: Math.floor(d.value * 80) })) },
  };
};
  ...
  return [
    {
      ...
      description: 'High memory usage detected on worker nodes. Auto-scaling triggered.',
    ...
    },
    {
      ...
      description: 'Deployment failed due to incompatible environment variables. Rolled back successfully.',
...
export const getServiceDetails = async (serviceId: string) => {
  ...
  return {
    ...
    recentLogs: [
      { id: 1, level: 'info', message: 'Service check successful', time: new Date().toISOString() },
      { id: 2, level: 'warning', message: 'Memory usage approaching threshold', time: new Date(Date.now() - 5*60000).toISOString() },
      { id: 3, level: 'info', message: 'Garbage collection completed', time: new Date(Date.now() - 15*60000).toISOString() },
    ],
...
export const toggleMaintenanceMode = async (serviceId: string, enable: boolean) => {
  await wait(1000);
  return { success: true, message: `Maintenance mode ${enable ? 'enabled' : 'disabled'} for ${serviceId}.` };
};
