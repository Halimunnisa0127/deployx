...
import { DomainsTableSkeleton, AnalyticsSkeleton } from '../components/DomainsSkeleton';
import { 
  NoDomainsEmptyState, 
...
export default function DomainsPage() {
  ...
  const counts = useMemo(() => {
    const res = { all: domains.length, verified: 0, pending: 0, failed: 0, 'ssl-expiring': 0 };
    domains.forEach(d => {
  ...
  const filteredDomains = useMemo(() => {
    ...
    return domains.filter(d => {
      if (activeFilter === 'ssl-expiring' && d.sslStatus !== 'expiring') return false;
      if (activeFilter !== 'all' && activeFilter !== 'ssl-expiring' && d.verificationStatus !== activeFilter) return false;
  ...
  return (
    <div className="space-y-6 md:space-y-8 pb-10 text-left animate-in fade-in duration-300">
      ...
      {loading && !refreshing ? (
        ...
        activeFilter === 'pending' ? <NoPendingDomainsEmptyState onClear={() => setActiveFilter('all')} /> :
        activeFilter === 'failed' ? <NoFailedDomainsEmptyState onClear={() => setActiveFilter('all')} /> :
