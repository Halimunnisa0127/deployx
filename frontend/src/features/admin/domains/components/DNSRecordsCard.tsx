          ...
          <tbody className="divide-y divide-slate-800/50">
            {records.map((rec, idx) => (
              <tr key={idx} className="hover:bg-slate-800/20 transition-colors">
                <td className="px-4 py-3 font-mono text-xs text-indigo-400">{rec.type}</td>
                <td className="px-4 py-3 font-mono text-xs text-slate-300">{rec.name}</td>
                <td className="px-4 py-3 font-mono text-xs text-slate-400 truncate max-w-[150px] sm:max-w-[200px]" title={rec.value}>
                  {rec.value}
                </td>
