...
export default function DomainsTable({ domains = [], onRowClick, actionHandlers }) {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;
  
  const totalPages = Math.ceil(domains.length / itemsPerPage);
  const paginatedData = domains.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
      ...
      {totalPages > 1 && (
        <div className="px-5 py-4 border-t border-slate-800/80 flex items-center justify-between text-sm bg-slate-900/40">
          <span className="text-slate-400">
            Showing {(currentPage - 1) * itemsPerPage + 1} to {Math.min(currentPage * itemsPerPage, domains.length)} of {domains.length}
          </span>
