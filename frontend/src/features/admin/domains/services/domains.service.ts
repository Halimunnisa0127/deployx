...
export const getDNSRecords = async (id: string) => {
  await wait(300);
  return [
    { type: 'A', name: '@', value: '76.76.21.21', status: 'valid' },
    { type: 'CNAME', name: 'www', value: 'cname.deployx.dev', status: 'valid' },
    { type: 'TXT', name: '_deployx-challenge', value: 'dx-verify-abc123xyz', status: 'pending' },
  ];
};

export const getSSLInfo = async (id: string) => {
  await wait(300);
  const domain = mockDomains.find(d => d.id === id);
  const statusMap = {
    'active': { status: 'Active', issued: '2026-05-01', expiry: '2026-08-01', daysRemaining: 2 },
    'expiring': { status: 'Expiring Soon', issued: '2026-05-05', expiry: '2026-08-05', daysRemaining: 6 },
    'expired': { status: 'Expired', issued: '2026-03-01', expiry: '2026-06-01', daysRemaining: 0 },
    'pending': { status: 'Pending Issuance', issued: null, expiry: null, daysRemaining: null },
  };
...
export const getVerificationHistory = async (id: string) => {
  await wait(300);
  return [
    { time: '2026-07-29T10:00:00Z', status: 'success', message: 'DNS records verified successfully' },
    { time: '2026-07-29T09:55:00Z', status: 'failed', message: 'Missing TXT record _deployx-challenge' },
    { time: '2026-07-29T09:50:00Z', status: 'pending', message: 'Verification initiated' }
  ];
};
...
export const refreshDomain = async (id: string) => {
  await wait(600);
